...
def test_create_stock():
    ...
    response = client.post(
        "/stocks/", json={"product_name": product, "quantity": 10}, headers=headers
    )
...
def test_create_stock_duplicate_product():
    ...
    client.post(
        "/stocks/", json={"product_name": product, "quantity": 5}, headers=headers
    )
    response = client.post(
        "/stocks/", json={"product_name": product, "quantity": 7}, headers=headers
    )
...
def test_create_stock_negative_quantity():
    ...
    response = client.post(
        "/stocks/", json={"product_name": product, "quantity": -1}, headers=headers
    )
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
...
def test_get_stock():
    ...
    create_resp = client.post(
        "/stocks/", json={"product_name": product, "quantity": 3}, headers=headers
    )
...
def test_update_stock():
    ...
    create_resp = client.post(
        "/stocks/", json={"product_name": product, "quantity": 2}, headers=headers
    )
    stock_id = create_resp.json()["id"]
...
def test_update_stock_duplicate_product():
    ...
    resp2 = client.post(
        "/stocks/", json={"product_name": product2, "quantity": 2}, headers=headers
    )
    id2 = resp2.json()["id"]
    response = client.put(
        f"/stocks/{id2}", json={"product_name": product1}, headers=headers
    )
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 400
...
def test_delete_stock():
    ...
    create_resp = client.post(
        "/stocks/", json={"product_name": product, "quantity": 4}, headers=headers
    )
    stock_id = create_resp.json()["id"]
