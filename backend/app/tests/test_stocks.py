...
def test_create_stock():
    ...
    response = client.post("/stocks/", json={"product_name": product, "quantity": 10}, headers=headers)
...
def test_create_stock_duplicate_product():
    ...
    client.post("/stocks/", json={"product_name": product, "quantity": 5}, headers=headers)
    response = client.post("/stocks/", json={"product_name": product, "quantity": 7}, headers=headers)
...
def test_create_stock_negative_quantity():
    ...
    response = client.post("/stocks/", json={"product_name": product, "quantity": -1}, headers=headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 422

def test_create_stock_empty_name():
    print(f"[TEST] test_create_stock_empty_name")
    response = client.post("/stocks/", json={"product_name": "", "quantity": 5}, headers=headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 422
...
def test_get_stock():
    ...
    create_resp = client.post("/stocks/", json={"product_name": product, "quantity": 3}, headers=headers)
...
def test_update_stock():
    ...
    create_resp = client.post("/stocks/", json={"product_name": product, "quantity": 2}, headers=headers)
    stock_id = create_resp.json()["id"]
    new_product = unique_product()
    response = client.put(f"/stocks/{stock_id}", json={"product_name": new_product, "quantity": 5}, headers=headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 200
...
def test_update_stock_duplicate_product():
    ...
    resp1 = client.post("/stocks/", json={"product_name": product1, "quantity": 1}, headers=headers)
    resp2 = client.post("/stocks/", json={"product_name": product2, "quantity": 2}, headers=headers)
    id2 = resp2.json()["id"]
    response = client.put(f"/stocks/{id2}", json={"product_name": product1}, headers=headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 400
...
def test_delete_stock():
    ...
    create_resp = client.post("/stocks/", json={"product_name": product, "quantity": 4}, headers=headers)
    stock_id = create_resp.json()["id"]
