...
class TestStockRoutesCRUD:
    ...
    def test_create_stock_missing_fields(self, client, auth_headers):
        ...
        response = client.post("/api/v1/stocks/", json=incomplete_stock, headers=auth_headers)
...
class TestOrderRoutesCRUD:
    ...
    def test_update_order_status(self, client, auth_headers):
        ...
        if create_response.status_code == 201:
            ...
            response = client.put(f"/api/v1/orders/{order_id}", 
                                json=update_data, headers=auth_headers)
            assert response.status_code == 200
            data = response.json()
...
class TestErrorHandling:
    ...
    def test_missing_content_type(self, client, auth_headers):
        ...
        headers_no_content = {k: v for k, v in auth_headers.items() if k != "Content-Type"}
        response = client.post("/api/v1/users/", 
                             json=user_data, 
