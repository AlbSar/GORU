...
class TestStockEdgeCasesFixed:
    ...
    def test_create_stock_zero_price(self, client, auth_headers):
        ...
        response = client.post("/api/v1/stocks/", json=zero_price_stock, headers=auth_headers)
...
class TestIntegrationWithAuth:
    ...
    def test_full_user_crud_with_auth(self, client, auth_headers):
        ...
        # Verify deletion
        get_deleted_response = client.get(f"/api/v1/users/{user_id}", headers=auth_headers)
    ...
    def test_full_stock_crud_with_auth(self, client, auth_headers):
        ...
        update_response = client.put(f"/api/v1/stocks/{stock_id}", json=update_data, headers=auth_headers)
        assert update_response.status_code == 200
