...
def test_create_user_missing_field(client, auth_headers):
    ...
    response = client.post("/api/v1/users/", json={"name": "No Email"}, headers=headers, headers=auth_headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 422
    response = client.post(
        "/api/v1/users/", json={"email": unique_email(, headers=auth_headers)}, headers=headers
    )
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 422
...
def test_get_nonexistent_user(client, auth_headers):
    ...
    response = client.get("/api/v1/users/9999999", headers=headers, headers=auth_headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
...
def test_list_users(client, auth_headers):
    ...
    response = client.get("/api/v1/users/", headers=headers, headers=auth_headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
...
def test_get_user(client, auth_headers):
    ...
    response = client.get(f"/api/v1/users/{user_id}", headers=headers, headers=auth_headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 200
    data = response.json()
...
def test_delete_user(client, auth_headers):
    ...
    , headers=auth_headers)
    user_id = user_resp.json()["id"]
    response = client.delete(f"/api/v1/users/{user_id}", headers=headers, headers=auth_headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
