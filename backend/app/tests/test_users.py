...
def test_create_user():
    ...
    response = client.post("/users/", json={"name": "Test User", "email": email}, headers=headers)
...
def test_create_user_invalid_email():
    ...
    response = client.post("/users/", json={"name": "Invalid Email", "email": "not-an-email"}, headers=headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 422
...
def test_create_user_duplicate_email():
    ...
    client.post("/users/", json={"name": "User1", "email": email}, headers=headers)
    response = client.post("/users/", json={"name": "User2", "email": email}, headers=headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
...
def test_get_user():
    ...
    user_resp = client.post("/users/", json={"name": "Detail User", "email": email}, headers=headers)
...
def test_update_user():
    ...
    user_resp = client.post("/users/", json={"name": "Update User", "email": email}, headers=headers)
    user_id = user_resp.json()["id"]
    new_email = unique_email()
    response = client.put(f"/users/{user_id}", json={"name": "Updated", "email": new_email}, headers=headers)
    print(f"[DEBUG] Response: {response.status_code}, {response.text}")
    assert response.status_code == 200
...
def test_delete_user():
    ...
    user_resp = client.post("/users/", json={"name": "Delete User", "email": email}, headers=headers)
    user_id = user_resp.json()["id"]
