...
def fix_linting_issues():
    ...
    success4 = run_command(
        "ruff check --select F401 --fix app/", "Unused imports removal"
    )
    ...
    success6 = run_command(
        "ruff check --select E203,W291,W292,W293 --fix app/", "Whitespace fixes"
    )
    ...
    success11 = run_command(
        "ruff check --select F821 --fix app/", "Undefined name fixes"
    )

    # 12. Local variable sorunlarını düzelt
    success12 = run_command(
        "ruff check --select F841 --fix app/", "Local variable fixes"
    )

    # 13. Module level import sorunlarını düzelt
    success13 = run_command(
        "ruff check --select E402 --fix app/", "Module level import fixes"
    )
